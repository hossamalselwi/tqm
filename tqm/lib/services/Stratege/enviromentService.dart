...
class EnviromentService {
  ...
  Future<List<EnviromentModel>> getData() async {
    idOrg = await _testIdorg();

    var url = "${SettingConfig.enviromentPath}/$idOrg";
    var result = await _api.getRequest(url);

    var data = json.decode(result.body) as List;

    var dataApi = new List<EnviromentModel>();
    for (int i = 0; i < data.length; i++) {
      var temp = EnviromentModel.fromJson(data[i]);
      dataApi.add(temp);
    }
  ...
  Future<bool> delete(int id) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.enviromentPath}/$id";
  ...
  Future<bool> update(EnviromentModel model) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.enviromentPath}/${model.id}";
      var response =
  ...
  Future<bool> add(EnviromentModel model) async {
    bool statues = false;
    model.id = '0';
    ...
    try {
      var response = await _api.postRequest(
          route: '${SettingConfig.enviromentPath}', body: model.toJson(model));
