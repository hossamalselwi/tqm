...
class UnitService {
  ...
  Future<List<UnitsModel>> getData() async {
    idOrg = await _testIdorg();

    var url = "${SettingConfig.unitPath}/$idOrg";
    var result = await _api.getRequest(url);

    var data = json.decode(result.body) as List;

    var dataApi = new List<UnitsModel>();
    for (int i = 0; i < data.length; i++) {
      var temp = UnitsModel.fromJson(data[i]);
      dataApi.add(temp);
    }
  ...
  Future<bool> delete(int id) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.unitPath}/$id";
  ...
  Future<bool> update(UnitsModel model) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.unitPath}/${model.id}";
      var response =
  ...
  Future<bool> add1({UnitsModel model}) async {
    bool statues = false;
    model.id = 0;
    ...
    try {
      var response = await _api.postRequest(
          route: '${SettingConfig.unitPath}', body: model.toJson(model));
