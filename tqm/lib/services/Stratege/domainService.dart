...
class DomainService {
  ...
  Future<List<DomainModel>> getData() async {
    idOrg = await _testIdorg();

    var url = "${SettingConfig.domainPath}/$idOrg";
    var result = await _api.getRequest(url);

    var data = json.decode(result.body) as List;

    var dataApi = new List<DomainModel>();
    for (int i = 0; i < data.length; i++) {
      var temp = DomainModel.fromJson(data[i]);
      dataApi.add(temp);
    }
  ...
  Future<bool> delete(int id) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.domainPath}/$id";
  ...
  Future<bool> update(DomainModel model) async {
    bool statues = false;
    try {
      var url = "${SettingConfig.domainPath}/${model.id}";
      var response =
  ...
  Future<bool> add(DomainModel model) async {
    bool statues = false;
    model.id = '0';
    ...
    try {
      var response = await _api.postRequest(
          route: '${SettingConfig.domainPath}', body: model.toJson(model));
